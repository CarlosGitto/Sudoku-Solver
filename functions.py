            ...
            for h in range(3):
                ...
                for ch in row:
                    for k in range(3):
                        ...
                        if j==9:
                            j=0
                        dict.update({"{}{}".format((ch),str(i)+str(j)) : 0})
                    
                        if sudoku[i,j]!=0:
                                dict.update({"{}{}".format((ch),str(i)+str(j)) : (sudoku[i][j])})
#===================================================================================#
#                           Dividen Sudoku                                          #
#                                                                                   #
#===================================================================================#
...
def put_val(dict,sudoku):
    for name, val in dict.items():
        ...
        j=int(name[2])
        ch=name[0]
        if type(val)==list and len(val)==1:     #actualiza un valor que posee una unica posibilidad en el diccionario
                x=val[0]                               # tambien actualiza el sudoku
                dict[name]=x              
                sudoku[i,j]=x
                print(name,":",x)
                ...
                for nm, value in dict.items():
                    if ch in nm and type(value)==list and x in value:     #quita el valor actualizado de la familia
                        value.remove(x)
                        print(nm,value)
                    else:
                        continue
...
def fnd_kn_val(sudoku,sudoku_row,sudoku_clmn,dict,i,j):
    n=[]
...
def val_x_y_sqr(sudoku,dict):
    for i in range(9):
        for j in range(9):
            if sudoku[i][j]==0:
                pos_val=fnd_kn_val(sudoku,sudoku[i],sudoku[:,j],dict,i,j)
                for name,val in dict.items():
